...
- Do not launch pods on CSU-TIDE reserved hardware. Never add or keep a `nautilus.io/reservation=csu-tide` toleration in any Kubernetes manifest, and stop immediately if a pod lands on a reserved `rci-tide-*` node.
