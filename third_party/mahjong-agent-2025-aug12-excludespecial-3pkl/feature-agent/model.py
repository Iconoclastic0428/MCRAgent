#!/usr/bin/env python
# -*- encoding: utf-8 -*-
# @Author  :   Arthals
# @File    :   model.py
# @Time    :   2024/06/30 18:40:46
# @Contact :   zhuozhiyongde@126.com
# @Software:   Visual Studio Code

"""
model.py: 用于定义神经网络模型
"""

import torch
import torch.nn.functional as F
from torch import nn


class MixedKernelInputLayer(nn.Module):
    def __init__(self, obs_dim, hidden=128, mixed_kernel_input=False):
        nn.Module.__init__(self)
        self.mixed_kernel_input = mixed_kernel_input
        if mixed_kernel_input:
            self._conv3 = nn.Sequential(
                nn.Conv2d(obs_dim, 128, 3, 1, 1, bias=False),
                nn.BatchNorm2d(128),
            )
            self._conv1 = nn.Sequential(
                nn.Conv2d(obs_dim, 128, 1, 1, 0, bias=False),
                nn.BatchNorm2d(128),
            )
            self._merge = nn.Sequential(
                nn.GELU(),
                nn.Conv2d(128, hidden, 3, 1, 1, bias=False),
                nn.BatchNorm2d(hidden),
                nn.GELU(),
            )
        else:
            self._layers = nn.Sequential(
                nn.Conv2d(obs_dim, 128, 3, 1, 1, bias=False),
                nn.BatchNorm2d(128),
                nn.GELU(),
                nn.Conv2d(128, hidden, 3, 1, 1, bias=False),
                nn.BatchNorm2d(hidden),
                nn.GELU(),
            )

    def forward(self, obs):
        if self.mixed_kernel_input:
            return self._merge(self._conv3(obs) + self._conv1(obs))
        return self._layers(obs)


class DuelingHead(nn.Module):
    def __init__(self, input_dim, hidden_dims, action_dim=235):
        nn.Module.__init__(self)
        layers = []
        current_dim = input_dim
        for hidden_dim in hidden_dims:
            layers.append(nn.Linear(current_dim, hidden_dim))
            layers.append(nn.GELU())
            current_dim = hidden_dim
        self._trunk = nn.Sequential(*layers)
        self._advantage = nn.Linear(current_dim, action_dim)
        self._value = nn.Linear(current_dim, 1)

    def forward(self, x):
        x = self._trunk(x)
        advantage = self._advantage(x)
        value = self._value(x)
        return value + advantage - advantage.mean(dim=1, keepdim=True)


class SelfVecModel(nn.Module):
    def __init__(
        self,
        obs_dim,
        vec_dim,
        hidden=128,
        num_blocks=20,
        mixed_kernel_input=False,
        dueling_head=False,
    ):
        nn.Module.__init__(self)
        self.hidden = hidden
        self.obs_dim = obs_dim
        self.vec_dim = vec_dim
        self.dueling_head = dueling_head
        self._input_layer = MixedKernelInputLayer(
            obs_dim, hidden=hidden, mixed_kernel_input=mixed_kernel_input
        )
        self._hidden_layers = nn.ModuleList(
            [self.res_block(self.hidden) for _ in range(num_blocks)]
        )

        down_sample_ratio = ((hidden * 4 * 9 + vec_dim) / 235) ** (1 / 3)
        down_sample_dim_1 = int((hidden * 4 * 9 + vec_dim) / down_sample_ratio)
        down_sample_dim_1 = down_sample_dim_1 // 8 * 8
        down_sample_dim_2 = int(down_sample_dim_1 / down_sample_ratio)
        down_sample_dim_2 = down_sample_dim_2 // 8 * 8

        head_input_dim = hidden * 4 * 9 + vec_dim
        if dueling_head:
            self._output_layer = DuelingHead(
                head_input_dim, (down_sample_dim_1, down_sample_dim_2)
            )
        else:
            self._output_layer = nn.Sequential(
                nn.Linear(head_input_dim, down_sample_dim_1),
                nn.GELU(),
                nn.Linear(down_sample_dim_1, down_sample_dim_2),
                nn.GELU(),
                nn.Linear(down_sample_dim_2, 235),
            )

        for m in self.modules():
            if isinstance(m, nn.Conv2d) or isinstance(m, nn.Linear):
                nn.init.kaiming_normal_(m.weight)

    def res_block(self, hidden=256):
        return nn.Sequential(
            nn.Conv2d(hidden, hidden, 3, 1, 1, bias=False),
            nn.BatchNorm2d(hidden),
            nn.GELU(),
            nn.Conv2d(hidden, hidden, 3, 1, 1, bias=False),
            nn.BatchNorm2d(hidden),
        )

    def forward(self, input_dict):
        self.train(mode=input_dict.get("is_training", False))
        obs = input_dict["obs"]["observation"].float()
        vec = input_dict["obs"]["vec"].float()
        x = self._input_layer(obs)
        for block in self._hidden_layers:
            x = x + block(x)
            x = F.gelu(x)
        # 展平, 128*4*9 = 4608
        x = torch.flatten(x, start_dim=1)
        # 链接 x 和 vec, 4608 + 117 = 4725
        x = torch.cat([x, vec], dim=1)
        # FC
        x = self._output_layer(x)

        action_mask = input_dict["obs"]["action_mask"].float()
        inf_mask = torch.clamp(torch.log(action_mask), -1e38, 1e38)
        return x + inf_mask


class SlideStyleModel(nn.Module):
    """Slide-style ResNet head adapted to the current feature-agent tensors."""

    def __init__(
        self,
        obs_dim,
        vec_dim,
        hidden=64,
        num_blocks=20,
        out_planes=8,
        slide_vec_dim=78,
        fc_hidden=256,
        mixed_kernel_input=False,
        dueling_head=False,
    ):
        nn.Module.__init__(self)
        self.hidden = hidden
        self.obs_dim = obs_dim
        self.vec_dim = vec_dim
        self.out_planes = out_planes
        self.slide_vec_dim = slide_vec_dim
        self.dueling_head = dueling_head

        self._input_layer = MixedKernelInputLayer(
            obs_dim, hidden=hidden, mixed_kernel_input=mixed_kernel_input
        )
        self._hidden_layers = nn.ModuleList(
            [self.res_block(self.hidden) for _ in range(num_blocks)]
        )
        self._projection = nn.Sequential(
            nn.Conv2d(hidden, out_planes, 1, 1, 0, bias=False),
            nn.BatchNorm2d(out_planes),
            nn.GELU(),
        )
        self._vec_adapter = (
            nn.Identity()
            if vec_dim == slide_vec_dim
            else nn.Sequential(nn.Linear(vec_dim, slide_vec_dim), nn.GELU())
        )
        head_input_dim = out_planes * 4 * 9 + slide_vec_dim
        if dueling_head:
            self._output_layer = DuelingHead(head_input_dim, (fc_hidden, fc_hidden))
        else:
            self._output_layer = nn.Sequential(
                nn.Linear(head_input_dim, fc_hidden),
                nn.GELU(),
                nn.Linear(fc_hidden, fc_hidden),
                nn.GELU(),
                nn.Linear(fc_hidden, 235),
            )

        for m in self.modules():
            if isinstance(m, nn.Conv2d) or isinstance(m, nn.Linear):
                nn.init.kaiming_normal_(m.weight)

    def res_block(self, hidden=64):
        return nn.Sequential(
            nn.Conv2d(hidden, hidden, 3, 1, 1, bias=False),
            nn.BatchNorm2d(hidden),
            nn.GELU(),
            nn.Conv2d(hidden, hidden, 3, 1, 1, bias=False),
            nn.BatchNorm2d(hidden),
        )

    def forward(self, input_dict):
        self.train(mode=input_dict.get("is_training", False))
        obs = input_dict["obs"]["observation"].float()
        vec = input_dict["obs"]["vec"].float()
        x = self._input_layer(obs)
        for block in self._hidden_layers:
            x = x + block(x)
            x = F.gelu(x)
        x = self._projection(x)
        x = torch.flatten(x, start_dim=1)
        vec = self._vec_adapter(vec)
        x = torch.cat([x, vec], dim=1)
        x = self._output_layer(x)

        action_mask = input_dict["obs"]["action_mask"].float()
        inf_mask = torch.clamp(torch.log(action_mask), -1e38, 1e38)
        return x + inf_mask
